...
function switchPage(pageId) {
    ...
    if (pageId !== 'home' && scannerState.running) {
        stopScannerCheckin({ hidePanel: true });
    }
...
let customAlert = {
    ...
    hide: function(options = {}) {
        ...
        const resetToDefaultLayout = () => {
            ...
            alertElement.classList.remove('no-backdrop-blur');
    ...
    acknowledge: function(title, message, buttonText = '') {
        return new Promise((resolve) => {
            const elements = this._getElements();
            if (!elements) {
                window.alert(`${title}\n${message}`);
    ...
    acknowledgeWithExtraAction: function(title, message, options = {}) {
        return new Promise((resolve) => {
            const elements = this._getElements();
            if (!elements) {
                if (typeof options.onExtra === 'function') {
                    try {
                        options.onExtra();
                    } catch (_) {
                        // ignore
            ...
            titleElement.textContent = title;
            messageElement.textContent = message;
            buttonElement.textContent = options.confirmText || t('common.confirm', '确定');
            cancelButtonElement.textContent = options.extraText || t('common.open_link', '跳转');
            ...
            const handleExtra = () => {
                if (typeof options.onExtra === 'function') {
                    try {
                        ...
                        console.warn('额外动作执行失败:', error);
                    }
                }
            };

            buttonElement.addEventListener('click', handleConfirm);
            cancelButtonElement.addEventListener('click', handleExtra);

            this._cleanup = () => {
                buttonElement.removeEventListener('click', handleConfirm);
                cancelButtonElement.removeEventListener('click', handleExtra);
            };
...
const SCANNER_QR_PREFIX = 'XMao.SYS';
const SCANNER_LOGIN_AUDIO_PATH = './XMao_Core/Login.mp3';
const SCANNER_DUPLICATE_COOLDOWN_MS = 1200;
const SCANNER_DETECT_INTERVAL_MS = 220;
const SCANNER_JSQR_LOCAL_PATH = './XMao_Core/vendor/jsQR.min.js';
const SCANNER_JSQR_REMOTE_PATHS = [
    'https://cdn.jsdelivr.net/npm/jsqr@1.4.0/dist/jsQR.min.js',
    'https://unpkg.com/jsqr@1.4.0/dist/jsQR.min.js'
];
let scannerUiInitialized = false;
let scannerEventBound = false;
let scannerWindowResizeBound = false;
let scannerLastDecoderErrorLogged = false;
let scannerPanelDragState = {
    active: false,
    pointerId: null,
    offsetX: 0,
    offsetY: 0
...
function sanitizeScannerField(rawValue) {
...
function normalizeScannerQQ(rawValue) {
...
function extractFirstHttpUrl(rawText) {
    ...
    try {
        const urlObj = new URL(matched[0]);
...
function parseScannerRating(rawValue) {
...
function buildScannerAvatarByQQ(qqValue) {
    const normalizedQQ = normalizeScannerQQ(qqValue);
    if (!normalizedQQ) return DEFAULT_AVATAR_PATH;
    return `https://q.qlogo.cn/g?b=qq&nk=${normalizedQQ}&s=640`;
}

function getScannerElements() {
    return {
        openBtn: document.getElementById('openScannerCheckinBtn'),
        cameraDialog: document.getElementById('scannerCameraDialog'),
        cameraSelect: document.getElementById('scannerCameraSelect'),
        cameraConfirm: document.getElementById('scannerCameraConfirm'),
        cameraCancel: document.getElementById('scannerCameraCancel'),
        panel: document.getElementById('scannerFloatPanel'),
        dragHandle: document.getElementById('scannerFloatDragHandle'),
        closeBtn: document.getElementById('scannerFloatCloseBtn'),
        openCreateQrBtn: document.getElementById('scannerOpenCreateQrBtn'),
        video: document.getElementById('scannerPreviewVideo'),
        status: document.getElementById('scannerFloatStatus'),
        dockToggle: document.getElementById('scannerFloatDockToggle')
    };
}

function setScannerAlertNoBlur(enabled) {
    const alertElement = document.getElementById('customAlert');
    if (!alertElement) return;
    alertElement.classList.toggle('no-backdrop-blur', !!enabled);
}

function waitForCustomAlertClose() {
    return new Promise((resolve) => {
        const alertElement = document.getElementById('customAlert');
...
async function showScannerSystemToast(message) {
    setScannerAlertNoBlur(true);
    if (typeof customAlert !== 'undefined' && customAlert && typeof customAlert.toast === 'function') {
        customAlert.toast(t('scanner.toast.title', '扫码检录器'), String(message || ''), 1000);
        await new Promise((resolve) => setTimeout(resolve, 1100));
        setScannerAlertNoBlur(false);
        return;
    }
...
async function showScannerInvalidAlert(rawContent) {
    setScannerAlertNoBlur(false);
    const contentText = String(rawContent || '');
    const jumpUrl = extractFirstHttpUrl(contentText);
    const messageText = `${t('scanner.invalid.message', '非系统内容，请核实后扫码。')}\n${t('scanner.invalid.raw', '二维码内容："{value}"', { value: contentText })}`;

    if (typeof customAlert !== 'undefined' && customAlert) {
        if (jumpUrl && typeof customAlert.acknowledgeWithExtraAction === 'function') {
            await customAlert.acknowledgeWithExtraAction(
                t('scanner.invalid.title', '非系统内容'),
                messageText,
                {
                    confirmText: t('common.confirm', '确定'),
                    extraText: t('common.open_link', '跳转'),
                    onExtra: () => {
        ...
        if (typeof customAlert.acknowledge === 'function') {
            await customAlert.acknowledge(
                t('scanner.invalid.title', '非系统内容'),
                messageText,
                t('common.confirm', '确定')
            );
        ...
        if (typeof customAlert.show === 'function') {
            customAlert.show(
                t('scanner.invalid.title', '非系统内容'),
                messageText,
                t('common.confirm', '确定')
    ...
    window.alert(`非系统内容，请核实后扫码。 二维码内容："${contentText}"`);
}

function updateScannerStatus(message, level = '') {
    const { status } = getScannerElements();
    if (!status) return;
    status.classList.remove('is-success', 'is-warn', 'is-error');
    if (level === 'success') status.classList.add('is-success');
    if (level === 'warn') status.classList.add('is-warn');
    if (level === 'error') status.classList.add('is-error');
    status.textContent = String(message || '');
}

function parseScannerSystemContent(rawContent) {
    const text = String(rawContent ?? '').trim();
    const matched = text.match(/^XMao\.SYS(Login|LogOut)\[(.*)\]$/i);
    if (!matched) {
        return null;
    }

    const action = String(matched[1] || '').toLowerCase() === 'logout' ? 'logout' : 'login';
    const payload = String(matched[2] || '');
    const fields = payload.split(',').map(item => sanitizeScannerField(item));
...
function findPersonIndexesByQQ(qqValue) {
    const normalizedQQ = normalizeScannerQQ(qqValue);
    if (!normalizedQQ) return [];

    const indexes = [];
    people.forEach((person, index) => {
        if (normalizeScannerQQ(person?.qq) === normalizedQQ) {
            indexes.push(index);
        }
    });
    return indexes;
...
function findPersonIndexForLogin(nameValue, qqValue) {
    const normalizedQQ = normalizeScannerQQ(qqValue);
    if (normalizedQQ) {
        const byQQ = people.findIndex(item => normalizeScannerQQ(item?.qq) === normalizedQQ);
        if (byQQ >= 0) return byQQ;
    }

    const normalizedName = sanitizeScannerField(nameValue);
    if (normalizedName) {
        const byName = people.findIndex(item => sanitizeScannerField(item?.name) === normalizedName);
...
async function playScannerLoginAudio() {
    try {
        if (!scannerState.loginAudio) {
            scannerState.loginAudio = new Audio(SCANNER_LOGIN_AUDIO_PATH);
            scannerState.loginAudio.preload = 'auto';
        }
        scannerState.loginAudio.currentTime = 0;
        await scannerState.loginAudio.play();
    } catch (error) {
        console.warn('播放检录音频失败:', error);
    }
}

function applyScannerLogin(fields) {
    const rawName = fields[0] || '';
    const rawQQ = fields[1] || '';
    const rawRating = fields[2] || '';

    const personName = sanitizeScannerField(rawName) || t('scanner.person.default_name', '未命名选手');
    const normalizedQQ = normalizeScannerQQ(rawQQ);
    const ratingValue = parseScannerRating(rawRating);
    const avatarPath = buildScannerAvatarByQQ(normalizedQQ);

    const existingIndex = findPersonIndexForLogin(personName, normalizedQQ);
    const nowText = new Date().toLocaleString();

    if (existingIndex >= 0) {
        const previous = people[existingIndex] || {};
        people[existingIndex] = {
            ...previous,
            name: personName || previous.name,
            qq: normalizedQQ || previous.qq || '',
            rating: ratingValue !== null ? ratingValue : (previous.rating ?? null),
            avatar: avatarPath || previous.avatar || DEFAULT_AVATAR_PATH,
            createdAt: previous.createdAt || nowText
        };
    } else {
        people.push({
    ...
    renderPeopleList();
    playScannerLoginAudio().catch(() => {});
    return `${personName}${t('scanner.person.joined', '已加入')}`;
}

function applyScannerLogout(fields) {
    const normalizedQQ = normalizeScannerQQ(fields[1] || fields[0] || '');
    if (!normalizedQQ) {
        return t('scanner.logout.qq_missing', '退赛二维码缺少可用的 QQ 号');
    }

    const targetIndexes = findPersonIndexesByQQ(normalizedQQ);
    if (targetIndexes.length <= 0) {
        return t('scanner.logout.not_found', '未匹配到该 QQ 的在场选手');
    }

    const removedPeople = [];
    people = people.filter((person) => {
        const matched = normalizeScannerQQ(person?.qq) === normalizedQQ;
        if (matched) {
    ...
    renderPeopleList();
    const targetName = sanitizeScannerField(removedPeople[0]?.name) || normalizedQQ;
    return `${targetName}${t('scanner.person.left', '人员已离开')}`;
}

async function processScannerPayload(rawContent) {
    const text = String(rawContent || '').trim();
    if (!text) return;

    if (!text.startsWith(SCANNER_QR_PREFIX)) {
        updateScannerStatus(t('scanner.status.invalid', '识别到非系统二维码，请确认后重试'), 'error');
        scannerState.dialogBlocking = true;
        await showScannerInvalidAlert(text);
        scannerState.dialogBlocking = false;
        return;
    }

    const parsed = parseScannerSystemContent(text);
    if (!parsed) {
        updateScannerStatus(t('scanner.status.invalid_format', '识别到系统前缀，但格式无效'), 'warn');
        scannerState.dialogBlocking = true;
        await showScannerSystemToast(t('scanner.status.invalid_format', '识别到系统前缀，但格式无效'));
        scannerState.dialogBlocking = false;
        return;
    }
    ...
    if (parsed.action === 'login') {
        resultMessage = applyScannerLogin(parsed.fields);
        updateScannerStatus(resultMessage, 'success');
    } else {
        resultMessage = applyScannerLogout(parsed.fields);
        updateScannerStatus(resultMessage, 'success');
    }

    scannerState.dialogBlocking = true;
    await showScannerSystemToast(resultMessage);
    scannerState.dialogBlocking = false;
}

async function handleScannerDecodeContent(rawContent) {
    const text = String(rawContent || '').trim();
    if (!text) return;

    const now = Date.now();
    if (scannerState.lastContent === text && (now - scannerState.lastContentAt) < SCANNER_DUPLICATE_COOLDOWN_MS) {
        return;
    }
    scannerState.lastContent = text;
    scannerState.lastContentAt = now;

    if (scannerState.processingContent) return;
    scannerState.processingContent = true;
    try {
        await processScannerPayload(text);
    } finally {
        scannerState.processingContent = false;
    }
}

function stopScannerLoop() {
    if (scannerState.scanLoopId) {
        cancelAnimationFrame(scannerState.scanLoopId);
        scannerState.scanLoopId = 0;
    }
    scannerState.scanBusy = false;
}

function scannerLoopTick() {
    ...
    const canDetect = (
        !scannerState.dialogBlocking
        && !scannerState.processingContent
        && !scannerState.scanBusy
        && scannerState.detector
        && video.readyState >= 2
        && (now - scannerState.lastDetectAt) >= SCANNER_DETECT_INTERVAL_MS
    );

    if (canDetect) {
        scannerState.scanBusy = true;
        scannerState.lastDetectAt = now;
        Promise.resolve(scannerState.detector.detect(video))
            .then((barcodes) => {
                if (!Array.isArray(barcodes) || barcodes.length <= 0) return;
                const rawValue = String(barcodes[0]?.rawValue || '').trim();
                if (!rawValue) return;
                handleScannerDecodeContent(rawValue).catch((error) => {
                    console.warn('处理二维码内容失败:', error);
                });
            })
            .catch((error) => {
                if (!scannerLastDecoderErrorLogged) {
                    scannerLastDecoderErrorLogged = true;
                    console.warn('二维码识别器调用失败:', error);
                }
            })
            .finally(() => {
                scannerState.scanBusy = false;
    ...
    scannerState.scanLoopId = requestAnimationFrame(scannerLoopTick);
}

function showScannerPanel() {
    const { panel } = getScannerElements();
    if (!panel) return;
...
function stopScannerStreamTracks() {
...
function stopScannerCheckin(options = {}) {
    const hidePanel = options.hidePanel !== false;
    scannerState.running = false;
    stopScannerLoop();
    stopScannerStreamTracks();
    scannerState.scanBusy = false;
    scannerState.lastDetectAt = 0;
    scannerState.dialogBlocking = false;
    scannerState.processingContent = false;
    scannerState.lastContent = '';
    scannerState.lastContentAt = 0;
...
function getPanelRectForDockCheck(panel) {
...
function applyScannerDockState(side) {
    const { panel, dockToggle } = getScannerElements();
    if (!panel || !dockToggle) return;

    if (side !== 'left' && side !== 'right') {
        panel.classList.remove('is-docked', 'is-docked-left', 'is-docked-right');
        scannerState.dockedSide = '';
        dockToggle.textContent = '';
        return;
    }

    scannerState.dockedSide = side;
    panel.classList.add('is-docked');
    panel.classList.remove('is-docked-left', 'is-docked-right');
    panel.classList.add(side === 'left' ? 'is-docked-left' : 'is-docked-right');
    dockToggle.textContent = side === 'left' ? '>' : '<';
}

function clampScannerPanelInsideViewport() {
    const { panel } = getScannerElements();
    if (!panel) return;
    if (scannerState.dockedSide) return;

    const viewportWidth = window.innerWidth;
    const viewportHeight = window.innerHeight;
    const panelRect = panel.getBoundingClientRect();

    const maxLeft = Math.max(0, viewportWidth - panelRect.width - 8);
    const maxTop = Math.max(0, viewportHeight - panelRect.height - 8);

    scannerState.panelLeft = clampNumber(scannerState.panelLeft, 0, maxLeft, 24);
    scannerState.panelTop = clampNumber(scannerState.panelTop, 0, maxTop, 120);
    panel.style.left = `${scannerState.panelLeft}px`;
    panel.style.top = `${scannerState.panelTop}px`;
    panel.style.right = 'auto';
}

function undockScannerPanel() {
    const { panel } = getScannerElements();
    if (!panel) return;
    if (!scannerState.dockedSide) return;

    const side = scannerState.dockedSide;
    applyScannerDockState('');
    panel.style.right = 'auto';

    if (side === 'right') {
        const panelWidth = 336;
        scannerState.panelLeft = Math.max(8, window.innerWidth - panelWidth - 24);
    } else {
        scannerState.panelLeft = 24;
    }
    scannerState.panelTop = Math.min(Math.max(scannerState.panelTop || 120, 8), Math.max(8, window.innerHeight - 290));
    clampScannerPanelInsideViewport();
}

function handleScannerDockOnRelease() {
    const { panel } = getScannerElements();
    if (!panel) return;

    const rect = getPanelRectForDockCheck(panel);
    const threshold = 10;
    const nearLeft = rect.left <= threshold;
    const nearRight = (window.innerWidth - (rect.left + rect.width)) <= threshold;

    if (nearLeft) {
        applyScannerDockState('left');
        panel.style.left = '0px';
        panel.style.right = 'auto';
        return;
    }
    if (nearRight) {
        applyScannerDockState('right');
...
function beginScannerPanelDrag(event) {
    const { panel, dragHandle } = getScannerElements();
    ...
    const rect = panel.getBoundingClientRect();
    scannerPanelDragState.active = true;
    scannerPanelDragState.pointerId = event.pointerId;
    scannerPanelDragState.offsetX = event.clientX - rect.left;
    scannerPanelDragState.offsetY = event.clientY - rect.top;
    scannerState.panelLeft = rect.left;
    scannerState.panelTop = rect.top;
    panel.classList.add('is-dragging');
    dragHandle.setPointerCapture(event.pointerId);
    event.preventDefault();
}

function moveScannerPanelDrag(event) {
    const { panel } = getScannerElements();
    if (!panel || !scannerPanelDragState.active) return;
    if (event.pointerId !== scannerPanelDragState.pointerId) return;

    const panelRect = panel.getBoundingClientRect();
    const nextLeft = event.clientX - scannerPanelDragState.offsetX;
    const nextTop = event.clientY - scannerPanelDragState.offsetY;
    const maxLeft = Math.max(0, window.innerWidth - panelRect.width);
    const maxTop = Math.max(0, window.innerHeight - panelRect.height);

    scannerState.panelLeft = clampNumber(nextLeft, 0, maxLeft, 24);
    scannerState.panelTop = clampNumber(nextTop, 0, maxTop, 120);
...
function endScannerPanelDrag(event) {
    const { panel, dragHandle } = getScannerElements();
    if (!panel || !scannerPanelDragState.active) return;
    if (event && event.pointerId !== scannerPanelDragState.pointerId) return;

    scannerPanelDragState.active = false;
    scannerPanelDragState.pointerId = null;
    ...
    handleScannerDockOnRelease();
}

function initScannerUi() {
    if (scannerUiInitialized) return;
    const {
        panel,
        dragHandle,
        closeBtn,
    ...
    if (!panel || !dragHandle || !closeBtn || !openCreateQrBtn || !dockToggle) {
        return;
    }
    ...
    dragHandle.addEventListener('pointerdown', beginScannerPanelDrag);
    window.addEventListener('pointermove', moveScannerPanelDrag);
    window.addEventListener('pointerup', endScannerPanelDrag);
    window.addEventListener('pointercancel', endScannerPanelDrag);

    closeBtn.addEventListener('click', () => {
        stopScannerCheckin({ hidePanel: true });
        updateScannerStatus(t('scanner.status.closed', '扫码检录器已关闭'), 'warn');
    });

    openCreateQrBtn.addEventListener('click', () => {
        window.open('./CreateQR.html', '_blank', 'noopener,noreferrer');
    });
    ...
    if (!scannerWindowResizeBound) {
        window.addEventListener('resize', () => {
            if (scannerState.dockedSide === 'right') {
                const { panel: scannerPanel } = getScannerElements();
                if (scannerPanel) {
                    scannerPanel.style.left = 'auto';
                    scannerPanel.style.right = '0px';
                }
            }
            if (scannerState.dockedSide === 'left') {
                const { panel: scannerPanel } = getScannerElements();
                if (scannerPanel) {
                    scannerPanel.style.left = '0px';
                    scannerPanel.style.right = 'auto';
                }
            }
            clampScannerPanelInsideViewport();
        });
        scannerWindowResizeBound = true;
    }

    scannerUiInitialized = true;
}

async function ensureJsQrDecoderAvailable() {
    if (typeof window.jsQR === 'function') {
        return window.jsQR;
    }

    const scriptCandidates = [SCANNER_JSQR_LOCAL_PATH, ...SCANNER_JSQR_REMOTE_PATHS];
    let lastError = null;
    ...
    for (const src of scriptCandidates) {
        try {
            ...
            lastError = error;
            console.warn(`jsQR 加载失败: ${src}`, error);
        }
    }

    throw createAppError(
        'APP-SCAN-001',
        t('scanner.error.unsupported', '当前浏览器不支持二维码识别，且兼容解码器加载失败。'),
        lastError
    );
}

function buildJsQrDetector() {
    const detect = async (videoElement) => {
        ...
        if (!scannerState.fallbackCanvas) {
            scannerState.fallbackCanvas = document.createElement('canvas');
        }
        const canvas = scannerState.fallbackCanvas;
        if (canvas.width !== width || canvas.height !== height) {
            canvas.width = width;
            canvas.height = height;
            scannerState.fallbackCanvasContext = null;
        }

        if (!scannerState.fallbackCanvasContext) {
            scannerState.fallbackCanvasContext = canvas.getContext('2d', { willReadFrequently: true }) || canvas.getContext('2d');
        }
        const ctx = scannerState.fallbackCanvasContext;
        if (!ctx) return [];
        ...
        try {
            ...
            console.warn('读取视频帧失败:', error);
            return [];
        }

        const decoded = window.jsQR(imageData.data, width, height, { inversionAttempts: 'attemptBoth' });
        if (!decoded || !decoded.data) {
            return [];
        }

        return [{
            rawValue: String(decoded.data || '')
        }];
    };
...
async function ensureQrDetectorReady() {
    if (scannerState.detector) return scannerState.detector;

    if (typeof BarcodeDetector !== 'undefined') {
        let barcodeDetectorUsable = true;
        try {
            if (typeof BarcodeDetector.getSupportedFormats === 'function') {
                const supportedFormats = await BarcodeDetector.getSupportedFormats();
                if (Array.isArray(supportedFormats) && supportedFormats.length > 0 && !supportedFormats.includes('qr_code')) {
                    barcodeDetectorUsable = false;
                }
            }
        } catch (error) {
            console.warn('读取 BarcodeDetector 支持格式失败，将尝试兼容模式:', error);
        }

        if (barcodeDetectorUsable) {
            try {
                scannerState.detector = new BarcodeDetector({ formats: ['qr_code'] });
                scannerState.detectorMode = 'barcode_detector';
                return scannerState.detector;
            } catch (error) {
                console.warn('BarcodeDetector 初始化失败，将尝试兼容模式:', error);
            }
        }
    }

    await ensureJsQrDecoderAvailable();
    scannerState.detector = buildJsQrDetector();
    scannerState.detectorMode = 'jsqr';
    return scannerState.detector;
}

async function listVideoInputDevices() {
    if (!navigator.mediaDevices || typeof navigator.mediaDevices.enumerateDevices !== 'function') {
        throw createAppError('APP-SCAN-003', t('scanner.error.media_missing', '当前浏览器不支持媒体设备访问。'));
    }

    let devices = await navigator.mediaDevices.enumerateDevices();
    let videoInputs = devices.filter(item => item.kind === 'videoinput');
    const hasLabel = videoInputs.some(item => String(item.label || '').trim().length > 0);

    if (!hasLabel && navigator.mediaDevices && typeof navigator.mediaDevices.getUserMedia === 'function') {
        try {
            ...
            console.warn('获取摄像头标签权限失败:', error);
        }
        devices = await navigator.mediaDevices.enumerateDevices();
        videoInputs = devices.filter(item => item.kind === 'videoinput');
    }

    return videoInputs.map((item, index) => ({
        deviceId: item.deviceId,
        label: String(item.label || '').trim() || `${t('scanner.camera.default_label', '摄像头')} ${index + 1}`
    }));
}

function openCameraSelectDialog(cameraItems) {
    return new Promise((resolve) => {
        const {
            cameraDialog,
            cameraSelect,
        ...
        if (!cameraDialog || !cameraSelect || !cameraConfirm || !cameraCancel) {
            resolve(cameraItems[0]?.deviceId || '');
            return;
        }
        ...
        cameraSelect.value = cameraItems[0]?.deviceId || '';

        const cleanup = () => {
            cameraConfirm.removeEventListener('click', handleConfirm);
            cameraCancel.removeEventListener('click', handleCancel);
            cameraDialog.removeEventListener('click', handleBackdrop);
            cameraDialog.classList.remove('show');
        };

        const handleConfirm = () => {
            const selected = String(cameraSelect.value || '').trim();
            cleanup();
            resolve(selected);
        };
        ...
        cameraConfirm.addEventListener('click', handleConfirm);
        cameraCancel.addEventListener('click', handleCancel);
        cameraDialog.addEventListener('click', handleBackdrop);
        cameraDialog.classList.add('show');
    });
}

async function startScannerByDevice(deviceId, deviceLabel = '') {
    const { video } = getScannerElements();
    if (!video) {
        throw createAppError('APP-SCAN-004', t('scanner.error.video_missing', '未找到扫码预览容器。'));
    }
    if (!navigator.mediaDevices || typeof navigator.mediaDevices.getUserMedia !== 'function') {
        throw createAppError('APP-SCAN-005', t('scanner.error.media_missing', '当前浏览器不支持媒体设备访问。'));
    }

    await ensureQrDetectorReady();
    stopScannerCheckin({ hidePanel: false });
    ...
    scannerState.currentDeviceId = deviceId || '';
    scannerState.selectedDeviceLabel = String(deviceLabel || '').trim();

    video.srcObject = stream;
    try {
        await video.play();
    } catch (error) {
        console.warn('启动扫码视频预览失败:', error);
    }
    ...
    scannerState.processingContent = false;
    scannerState.dialogBlocking = false;
    scannerLastDecoderErrorLogged = false;

    showScannerPanel();
    applyScannerDockState('');
    clampScannerPanelInsideViewport();
    const readyMessage = scannerState.detectorMode === 'jsqr'
        ? t('scanner.status.ready_fallback', '扫码检录器已启动（兼容识别模式），正在等待二维码...')
        : t('scanner.status.ready', '扫码检录器已启动，正在等待二维码...');
    updateScannerStatus(
        readyMessage,
        'success'
    );
...
async function openScannerCheckin() {
    try {
        initScannerUi();

        const cameraItems = await listVideoInputDevices();
        if (!Array.isArray(cameraItems) || cameraItems.length <= 0) {
            throw createAppError('APP-SCAN-006', t('scanner.error.no_camera', '未检测到可用摄像头设备。'));
        }

        const selectedDeviceId = await openCameraSelectDialog(cameraItems);
        if (!selectedDeviceId) {
            updateScannerStatus(t('scanner.status.cancelled', '已取消摄像头选择'), 'warn');
            return;
        }

        const matchedDevice = cameraItems.find(item => item.deviceId === selectedDeviceId) || cameraItems[0];
        await startScannerByDevice(selectedDeviceId, matchedDevice?.label || '');
    } catch (error) {
        const finalError = error?.code ? error : createAppError('APP-SCAN-007', t('scanner.error.start_failed', '扫码检录器启动失败'), error);
        updateScannerStatus(formatAppError(finalError), 'error');
        if (typeof customAlert !== 'undefined' && customAlert && typeof customAlert.show === 'function') {
            customAlert.show(t('scanner.error.title', '扫码检录器'), formatAppError(finalError));
        } else {
            window.alert(formatAppError(finalError));
        }
    }
}
...
function initPeopleManagement() {
    ...
    // 导入导出按钮事件监听
    const openScannerCheckinBtn = document.getElementById('openScannerCheckinBtn');
    ...
    if (openScannerCheckinBtn) openScannerCheckinBtn.addEventListener('click', openScannerCheckin);
    ...
    initScannerUi();
    if (!scannerEventBound) {
        window.addEventListener('beforeunload', () => {
            stopScannerCheckin({ hidePanel: false });
        });
        scannerEventBound = true;
    }
...
// 删除人员（带确认）
async function confirmAndDeletePerson(personId) {
    const targetIndex = people.findIndex(item => String(item?.id ?? '') === String(personId ?? ''));
    if (targetIndex < 0) return;

    const person = people[targetIndex] || {};
    const personName = String(person.name || t('results.card.unknown_player', '未知选手'));

    let confirmed = false;
    if (typeof customAlert !== 'undefined' && customAlert && typeof customAlert.confirm === 'function') {
        confirmed = await customAlert.confirm(
            t('people.delete.confirm_title', '删除人员'),
            t('people.delete.confirm_message', '确定要删除人员「{name}」吗？', { name: personName }),
            t('common.confirm', '确定'),
            t('common.cancel', '取消')
        );
    } else {
        confirmed = window.confirm(`确定要删除人员「${personName}」吗？`);
    }
    ...
    if (typeof customAlert !== 'undefined' && customAlert) {
        if (typeof customAlert.toast === 'function') {
            customAlert.toast(
                t('common.success', '成功'),
                t('people.delete.success', '人员已删除'),
                1200
            );
        } else if (typeof customAlert.show === 'function') {
            customAlert.show(t('common.success', '成功'), t('people.delete.success', '人员已删除'));
        }
    }
}
...
function renderPeopleList() {
    ...
    people.forEach(person => {
        ...
        deleteBtn.className = 'person-item-delete-btn';
        deleteBtn.innerHTML = '&times;';
        deleteBtn.title = t('people.delete.button_title', '删除人员');
        deleteBtn.setAttribute('aria-label', t('people.delete.aria', '删除人员'));
        deleteBtn.addEventListener('click', async (event) => {
            event.preventDefault();
            event.stopPropagation();
            await confirmAndDeletePerson(person.id);
        });
